...
export function detectNumberFromLandmarks(landmarks: NormalizedLandmark[]) {
  ...
  const thumbBent = fingerIsBent(0, [1, 2, 3, 4], landmarks, 0.6);
  const indexBent = fingerIsBent(0, [5, 6, 7, 8], landmarks, 0.7);
  const middleBent = fingerIsBent(0, [9, 10, 11, 12], landmarks, 0.7);
  const ringBent = fingerIsBent(0, [13, 14, 15, 16], landmarks, 0.7);
  const pinkyBent = fingerIsBent(0, [17, 18, 19, 20], landmarks, 0.7);
